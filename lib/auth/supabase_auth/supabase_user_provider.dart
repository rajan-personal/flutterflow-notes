...
class MysaveSupabaseUser extends BaseAuthUser {
  MysaveSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> mysaveSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = MysaveSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
